...
type ImportMessages = { box: string; empty: string; missingColumns: string }

export function boxesFromCsv(text: string, messages: ImportMessages): BoxType[] {
  ...
  if (!headers) throw new Error(messages.empty)
  const columns = columnIndexes(headers)
  if (columns.length === undefined || columns.width === undefined || columns.height === undefined) throw new Error(messages.missingColumns)
  return rows.flatMap((row, index) => {
    ...
    return [{
      id: crypto.randomUUID(), name: row[columns.name ?? -1]?.trim() || `${messages.box} ${index + 1}`,
