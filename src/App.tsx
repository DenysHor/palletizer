...
export default function App() {
  ...
  const [initialBoxState] = useState<BoxType[]>(freshInitialBoxes)
  ...
  const newOrder = () => {
    ...
    const newBoxes = freshInitialBoxes()
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      await exportPalletPdf({ orderName, pallet: calculatedPallet, boxes: calculationInput.boxes, pallets: calculation.pallets, totalWeight: calculation.totalWeight, captures })
    } catch (error) {
  ...
  return <main>
    <header><div><p className="eyebrow">MVP</p><h1>Калькулятор палетизації</h1><p>Вкажіть габарити, а ми покажемо базову схему укладки.</p></div><span className={!needsRecalculation && meetsFillTarget ? 'status good' : 'status'}>{needsRecalculation ? 'Треба перерозкласти' : !allPlaced ? 'Частина коробок не вмістилась' : targetIsImpossibleByVolume ? `${targetFillPercent}% неможливо для ${calculation.pallets.length} палет` : underfilledPallets.length ? `Нижче ${targetFillPercent}%: ${underfilledPallets.length} палет` : `Розкладено на палетах: ${calculation.pallets.length}`}</span></header>
    <div className="layout">
      <section className="controls">
        ...
        <div className="calculation-actions"><button className="calculate" onClick={calculateLayout}>Розрахувати</button>{needsRecalculation && <span>Параметри змінено — натисніть «Розрахувати».</span>}</div>
      ...
      <section className="result"><div className="result-head"><div><p className="eyebrow">3D-схема</p><h2>{orderName ? `Укладка: ${orderName}` : 'Укладка на палеті'}</h2></div><div className="result-actions"><p>Перетягуйте, щоб повернути модель</p><button className="pdf-export" disabled={!calculation.pallets.length || isExportingPdf || needsRecalculation} onClick={exportPdf}>{isExportingPdf ? 'Створюємо PDF…' : 'Експортувати PDF'}</button></div></div>
        ...
        <div className="metrics"><Metric label="Розміщено на цій палеті" value={`${activePallet.placements.length} шт.`} /><Metric label="Висота вантажу" value={`${activePallet.usedHeight} мм`} /><Metric label="Вага вантажу" value={formatWeight(activePallet.totalWeight)} /></div>
        <section className="report"><div><p className="eyebrow">Звіт</p><h3>Підсумок замовлення</h3></div><div className="report-grid"><Metric label="Усього коробок" value={`${totalBoxes} шт.`} /><Metric label="Загальний об'єм" value={formatVolume(totalVolume)} /><Metric label="Палет потрібно" value={`${calculation.pallets.length} шт.`} /><Metric label="Вага коробок" value={formatWeight(calculation.totalWeight)} /><Metric label="Вага палет" value={formatWeight(palletsWeight)} /><Metric label="Вага з палетами" value={formatWeight(grossWeight)} /></div>{targetIsImpossibleByVolume && <p className="target-warning">За об’ємом {targetFillPercent}% можливі лише для схеми до {maximumPalletsAtTarget} палет, якщо остання палета є винятком. Поточній геометрії потрібно {calculation.pallets.length} палет.</p>}</section>
        <div className="pallet-contents"><h3>На цій палеті</h3><p>Наведіть на тип коробки, щоб підсвітити його у 3D.</p>{activeContents.map(({ box, index, quantity }) => <div className="pallet-content-row" key={box.id} tabIndex={0} onMouseEnter={() => setHighlightedBoxId(box.id)} onMouseLeave={() => setHighlightedBoxId(undefined)} onFocus={() => setHighlightedBoxId(box.id)} onBlur={() => setHighlightedBoxId(undefined)}><span className="colour-dot" style={{ backgroundColor: colourForIndex(index) }} aria-hidden="true" /><span>{box.name} ({box.length} × {box.width} × {box.height} мм)</span><strong>{quantity} шт.</strong></div>)}</div>
