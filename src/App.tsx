...
export default function App() {
  ...
  return <main>
    <header><div><p className="eyebrow">MVP</p><h1>Калькулятор палетизації</h1><p>Вкажіть габарити, а ми покажемо базову схему укладки.</p></div><span className={!needsRecalculation && meetsFillTarget ? 'status good' : 'status'}>{needsRecalculation ? 'Треба перерозкласти' : !allPlaced ? 'Частина коробок не вмістилась' : underfilledPallets.length ? `Нижче ${targetFillPercent}%: ${underfilledPallets.length} палет` : `Розкладено на палетах: ${calculation.pallets.length}`}</span></header>
